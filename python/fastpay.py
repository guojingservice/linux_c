...
CARD_INFO=[
    {
        "Name":"岳文斌",
        "IdNo":"422201198510225933",
        "CardNo":"6229014800433107",
        "Phone":"13528802757",
        "Cvn2":"912",
        "ExpDate":"1021",
    },
    {
        "Name":"郭靖",
        "IdNo":"421083199005220477",
        "CardNo":"6214837551855632",
        "Phone":"13530749353",
        "Cvn2":"",
        "ExpDate":"",
    },
...
class FastPayIns(FastPayApi):
    ...
    def payInit(self):
        ...
        body["Phone"] = self.cardInfo["Phone"]
        accountName = self.cardInfo["Name"]
        body["AccountName"] = accountName.decode("utf8").encode("utf8")
        body["CertNo"] = self.cardInfo["IdNo"]
        body["AccountNo"] = self.cardInfo["CardNo"]
        body["Password"] = ""
        body["OrderNum"] = orderId
        body["Cvn2"] = self.cardInfo["Cvn2"]
        body["ExpDate"] = self.cardInfo["ExpDate"]
        body["Amount"] = ""
        ...
        if(respBodyDict["ResultCode"] == "0000"):
            ...
            print respBodyDict["TradeStatus"]
            print "errmsg:[%s]" %respBodyDict["ErrMsg"]
            print respBodyDict["OrderNum"]
        else:
    ...
    def payConfirm(self):
        ...
        body["PhoneVeriCode"] = ""
        body["SmsOrderNum"] = ""
        body["AccountName"] = self.cardInfo["Name"]
        body["CertNo"] = self.cardInfo["IdNo"]
        body["AccountNo"] = self.cardInfo["CardNo"]
        body["Password"] = ""
        body["Cvn2"] = self.cardInfo["Cvn2"]
        ...
        initDataList = []
        self.loadInitData(initDataList)        
        cpOrderId = initDataList[0]
        orderTime = initDataList[1]
        smsOrderId = initDataList[2]
        amount = initDataList[3]
        if cpOrderId:
            body["OrderNum"] = cpOrderId
        if orderTime:
            body["OrderTime"] = orderTime
        if smsOrderId:
            body["SmsOrderNum"] = smsOrderId
        if amount:
            body["Amount"] = amount
        resp = self.processRequest(body)
        print ("res:[%s]" % resp)
    ...
    def queryOrder(self):
        ...
        body["OrderNum"] = "fast_FastIns20170628140706"
        #body["OrderNum"] = cpOrderId
        ...
        print ("res:[%s]"%resp)
        deBody = self.decodeBody(respDict["body"]) 
        print("bodyText:[%s]"% deBody)
...
if __name__=='__main__':
    ...
    if(len(sys.argv) != 2):
        ...
        print "example: python fastpaytest.py order"
        exit(0)
    if sys.argv[1] == "order":
        ins.payInit()
    elif sys.argv[1] == "pay":
